...
win = False     #keep track of winning, per level. Set to False to begin each level, True when a level is won.
current_level = 1   #the game starts with level 1.
max_levels = 2
level_list = [5,10]     #This is supposed to be empty and populated with functions. Hard coded for debugging purposes.
...
def screen_setup(current_lev, t_in_row):
    ...
    '''Not an actual function yet, but I want to add a function to draw the Level # at the top of the screen, very large.
    write_level_num()'''
...
\n2. Play a brand new level #{current_level}, with all your lives back.")
    choice = input(f'Type 1 for the first option, 2 for the second option.   ')
    if choice==1:
        reset_current_level()
    elif choice==2:
        #this function does not exist yet.
        print('call play_one_level()')
...
################################################
XCursors()
screen_setup(current_level, turt_in_row)
...
wn.onkeypress(turn_off_turtle_events, 'z')
