...
#Next goal: add in numbers for top and left to play the game (currently, only pseudocoded)
...
def get_row_numbers():
    ...
    for j in range(turt_in_row):        #loop over every row
        temp_sum = 0
        print('new row')
        #loop over every value in the row
        for i in range(turt_in_row):        #loop over every cell in the row
            ...
            if globals()['answer'][j][i]=='1' and i != (turt_in_row - 1):
                ...
                globals()[f'row_nums_list_{j}'].append(temp_sum)
                temp_sum = 0
    ...
    '''pseudocode:
    #I need to uniquely name them, so I can refer to each row later when drawing the numbers on the left. (j refers to the row num)
    First, create empty row_nums_list{j} for each row 
    given a row of the answer:
        for each cell in the row:
            if a given cell is a 1, 
                add 1 to a temp_sum
            if a given cell is 0 and the temp_sum is non-zero
                append temp_sum to the row_nums_list{j} 
    '''
...
def write_numbers(): #need a parameter??
    ...
    #this nested loop is used to:
    #1. create a string list for each row of row_nums
    #2. create strings for each value in each row of row_nums
    #3. append these strings onto the string list for that row
    for j in range(len(row_nums)):          
        globals()[f'row_nums_list_string_{j}']=[]
        for num in row_nums[j]:
            globals()[f'row_nums_list_string_{j}']+= str(num)
        print("string list for row",j, "is", globals()[f'row_nums_list_string_{j}'])

    #actually drawing the row numbers on the left
    for j in range(len(row_nums)):
        drawer.goto(-.75*turtle_gap-board_shift, j*turtle_gap-board_shift-num_shift)
        drawer.write(" ".join(globals()[f'row_nums_list_string_{j}']), align = 'right', font = font_setup)
