    ...
    for j in range(len(board)):        #loop over every row
        ...
        for i in range(len(board)):        #loop over every index in the row
            ...
            if board[j][i]=='1' and i != (turt_in_row - 1):
                ...
                globals()[f'row_nums_list_{j}'].append(str(temp_sum))
                temp_sum = 0
            elif i == (len(board)-1) and temp_sum == 0:
                globals()[f'row_nums_list_{j}'].append(str(temp_sum))
...
######      UNFINISHED FUNCTION - ONLY FOR TESTING SIZE OF THE HEARTS       ######
def place_lives():
...
# wn.onkeypress(lose_a_life, 'l') #debugging life loss. Need to change this to trigger with getting answer wrong.
